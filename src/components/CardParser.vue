...
const parseDSWeb = async (csv) => {
  ...
  const migrated = ['mb1', 'fmb1', 'pagl', 'phed']
  parsed.data.forEach(row => {
    ...
    let key = null;
    if (!meta.setIds.has(setCode) && !migrated.includes(setCode)) {
...
<template>
  <div class="root">
    ...
    <Multiselect v-if="collections.open.length > 1" v-model="upload.name" :options="collections.open" mode="single" />

    <Multiselect v-model="upload.format" placeholder="Format" :options="Object.keys(formats)" :can-clear="false" />

    <input v-if="formats[upload.format][0] === 'file'" id="file-input" type="file" @change="fileChange"
    ...
    <CardSearch v-if="formats[upload.format][0] === 'search'" @selected="handleSearch" />

    <div class="buttons">
      <label class="button" for="file-input" v-if="formats[upload.format][0] === 'file'">Choose file</label>
      <button v-if="!upload.active && upload.format && (upload.text || file.path) && upload.format !== 'Search'"
        @click="formats[upload.format][1]()" :disabled="file.path === null && upload.text === ''">
        Add
      </button>
    </div>
    ...
    <div class="progress" v-if="upload.active">
      ...
      <div class="bar" :style="{ width: upload.progress + '%' }" />
    </div>
  </div>
  <ul v-if="upload.errors.length > 0" class="errors">
    <li v-for="(error, index) in upload.errors" :key="index">
      {{ error }}
    </li>
  </ul>
</template>
