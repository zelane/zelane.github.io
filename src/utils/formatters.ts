...
export const cardsToText = async (cards: Card[], format: string) => {
  let list = "";
  if (format === 'mtgo') {
    for (const card of cards.values()) {
      list += `${card.count || 1} ${card.data.name} \n`;
    };
  }
  else if (format === 'mtga') {
    for (const card of cards.values()) {
      list += `${card.count || 1} ${card.data.name} (${card.data.set}) ${card.data.collector_number}\n`;
    };
  }
  // else if (format === 'mkm') {
  ...
  // }
    ...
    for (const card of cards.values()) {
      list += `"${card.count || 1}","0","${card.data.name}","${card.data.set}","Near Mint","English",${card.finish},"","2022-03-22 02:52:33.210000","${card.data.collector_number}"\n`;
    };
  }
  return list;
};
